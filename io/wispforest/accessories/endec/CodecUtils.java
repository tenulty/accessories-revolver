    ...
    public static <T> Endec<T> ofCodec(Codec<T> codec) {
        return Endec.of(
                (ctx, serializer, value) -> {
                    EdmEndec.INSTANCE.encode(ctx, serializer, Util.m_260975_(codec.encodeStart(createEdmOps(ctx), value), IllegalStateException::new));
                },
                (ctx, deserializer) -> {
                    return Util.m_260975_(codec.parse(createEdmOps(ctx), EdmEndec.INSTANCE.decode(ctx, deserializer)), IllegalStateException::new);
                }
        );
    }
            ...
            public <T1> DataResult<T> decode(DynamicOps<T1> ops, MapLike<T1> input) {
                return captureThrows(() -> {
                    ...
                    input.entries().forEach(pair -> {
                        map.put(
                                Util.m_260975_(ops.getStringValue(pair.getFirst()), s -> new IllegalStateException("Unable to parse key: " + s)),
