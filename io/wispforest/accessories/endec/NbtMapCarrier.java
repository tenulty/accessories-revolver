...
public record NbtMapCarrier(CompoundTag compoundTag) implements MapCarrier {
    ...
    public <T> T getWithErrors(SerializationContext ctx, @NotNull KeyedEndec<T> key) {
        ...
        return key.endec().decodeFully(ctx.withAttributes(SerializationAttributes.HUMAN_READABLE), NbtDeserializer::of, this.compoundTag().m_128423_(key.key()));
    }

    @Override
    public <T> void put(SerializationContext ctx, @NotNull KeyedEndec<T> key, @NotNull T value) {
        this.compoundTag().m_128365_(key.key(), key.endec().encodeFully(ctx.withAttributes(SerializationAttributes.HUMAN_READABLE), NbtSerializer::of, value));
    }

    @Override
    public <T> void delete(@NotNull KeyedEndec<T> key) {
        this.compoundTag().m_128473_(key.key());
    }

    @Override
    public <T> boolean has(@NotNull KeyedEndec<T> key) {
        return this.compoundTag().m_128441_(key.key());
    }
}
