...
public class SlotTypeLoader extends ReplaceableJsonResourceReloadListener {
    ...
    public static SlotType getSlotType(LivingEntity entity, String slotName){
        return getSlotTypes(entity.m_9236_()).get(slotName);
    }
    ...
    public static Map<String, SlotType> getSlotTypes(Level level){
        return INSTANCE.getSlotTypes(level.m_5776_());
    }
    ...
    public static Collection<SlotType> getUsedSlotsByRegistryItem(LivingEntity living){
        var map = getUsedSlots(living.m_9236_().m_5776_());

        if (map.containsKey(living.m_6095_())) return map.get(living.m_6095_());

        var validSlotTypes = new HashSet<SlotType>();

        BuiltInRegistries.f_257033_.forEach(item -> {
            var stack = item.m_7968_();

            var validSlots = AccessoriesAPI.getStackSlotTypes(living.m_9236_(), stack);

            validSlotTypes.addAll(validSlots);
        });

        map.put(living.m_6095_(), validSlotTypes);
    ...
    protected void apply(Map<ResourceLocation, JsonObject> data, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        for (var resourceEntry : data.entrySet()) {
            ...
            var pathParts = location.m_135815_().split("/");
            ...
            slotBuilder.icon(safeHelper((object, s) -> ResourceLocation.m_135820_(GsonHelper.m_13906_(object, s)), jsonObject, "icon", location));

            slotBuilder.order(this.<Integer>safeHelper(GsonHelper::m_13927_, jsonObject, "order", location));

            if(ExtraSlotTypeProperties.getProperty(slotBuilder.name, false).allowResizing()){
                var amount = this.safeHelper(GsonHelper::m_13927_, jsonObject, "amount", location);

                if(amount != null) {
                    var operation = this.safeHelper((jsonObject1, s) -> {
                        try {
                            return OperationType.valueOf(GsonHelper.m_13906_(jsonObject1, s).toUpperCase(Locale.ROOT));
            ...
            if(!ExtraSlotTypeProperties.getProperty(slotBuilder.name, false).strictMode()) {
                var validators = safeHelper(GsonHelper::m_13933_, jsonObject, "validators", new JsonArray(), location);

                decodeJsonArray(validators, "validator", location, element -> ResourceLocation.m_135820_(element.getAsString()), slotBuilder::validator);
            }

            slotBuilder.dropRule(this.safeHelper((object, s) -> DropRule.valueOf(GsonHelper.m_13906_(object, s)), jsonObject, "drop_rule", location));
