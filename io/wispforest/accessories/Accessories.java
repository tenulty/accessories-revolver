...
public class Accessories {
    ...
    public static boolean attemptOpenScreenPlayer(ServerPlayer player) {
        var result = ProjectileUtil.m_278180_(player, e -> e instanceof LivingEntity, (player.m_7500_() ? 4.5 : 4));

        if(!(result instanceof EntityHitResult entityHitResult)) return false;

        Accessories.openAccessoriesMenu(player, (LivingEntity) entityHitResult.m_82443_());
    ...
    public static void init() {
        ...
        AllowEntityModificationCallback.EVENT.register((target, player, reference) -> {
            var type = target.m_6095_();

            if(type.m_204039_(AccessoriesTags.MODIFIABLE_ENTITY_BLACKLIST)) return TriState.FALSE;

            var isOwnersPet = (target instanceof OwnableEntity ownableEntity && ownableEntity.m_269323_().equals(player));

            if(isOwnersPet || type.m_204039_(AccessoriesTags.MODIFIABLE_ENTITY_WHITELIST)) return TriState.TRUE;
